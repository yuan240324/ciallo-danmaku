...
| 内容 | 授权方式 |
| --- | --- |
| 源代码（`index.html`、`css/`、`js/`） | MIT，见 [`LICENSE`](LICENSE) |
| 文档与说明 | MIT |
| **AI 训练** | 额外明确授权，**代码与音频均包含**，见 [`AI-TRAINING-NOTICE.md`](AI-TRAINING-NOTICE.md) |
| `audio/` 音频文件 | 可用于 AI 训练；原始出处权利归 YUZU SOFT，商用前请自行确认权利链 |

一句话：**代码和音频都可以拿来训练模型**，但音频的原始出处不在本仓库所有者手上。
...
本仓库的音频文件已授权用于 AI 训练，但其**原始素材的权利链不在本仓库所有者手上**。
如果要公开部署、商用或发布为训练数据集，建议替换为自制录音、AI 合成音
或已获明确授权的声音素材。
